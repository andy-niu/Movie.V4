...
namespace M.ServiceAPI.Controllers
{
    ...
    public class UserRoleController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetList(UserRole model, int page = 1, int pageSize = 10)
        {
            try
            {
        ...
        [HttpPost("save")]
        public async Task<IActionResult> Save([FromBody]UserRole model)
        {
            try
            {
        ...
        [HttpPut("edit")]
        public async Task<IActionResult> Edit([FromBody]UserRole model)
        {
            try
            {
        ...
        [HttpDelete("destory")]
        public async Task<IActionResult> Destory([FromBody]UserRole model)
        {
            try
            {
        ...
        public async Task<IActionResult> Destory(Guid id)
        {
            try
            {
                var result = await _service.Delete(model => model.RoleId == id);
                var returnReult = new ApiResult(ApiResultCode.Success, "Sccuessfully.", "操作成功", result);
                return Ok(returnReult);
            }
            catch (Exception ex)
